...
def compute_mae(target, pred, mask):
...
def compute_mlm(pl_module, batch):
    use_mae = pl_module.hparams.config["use_mae"]
    mae_weight = pl_module.hparams.config["mae_weight"]
    infer = pl_module.infer(batch, mask_text=True, use_mae=use_mae)
    ...
    if use_mae:
        ...
        pred = infer["mae_pred"]
        mask = infer["mae_mask"]
        mae_loss = compute_mae(target, pred, mask)
        ret["mae_loss"] = mae_weight * mae_loss
...
def compute_irtr_recall(pl_module):
    ...
    for _b in tqdm.tqdm(image_loader, desc="image prefetch loop"):
        (ie, _, _) = pl_module.transformer.visual_embed(
...
def interpolate_pos_embed(model, checkpoint_model, is_pretrain=False):
    ...
    if is_pretrain:
        ...
        pos_embed_key = 'transformer.pos_embed'
        decoder_pos_embed_key = 'transformer.decoder_pos_embed'
